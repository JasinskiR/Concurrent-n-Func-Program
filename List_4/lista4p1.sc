...
class SavingsAccount(initialBalance: Double) extends BankAccount(initialBalance):
  ...
  override def deposit(amount: Double): Double =
    operation_acc += 1
    super.deposit(if operation_acc <= free_acc_operation then amount else amount - 1)
  override def withdraw(amount: Double): Double =
    operation_acc += 1
    super.withdraw(if operation_acc <= free_acc_operation then amount else amount + 1)
...
class Pies(imie: String = "") extends Zwierz(imie):
...
// b
class Kot(imie: String = "") extends Zwierz(imie):
  override def rodzaj(): String = "Kot"
  override def dajGlos(): String = "Miau, miau!"


class Krowa(imie: String = "") extends Zwierz(imie):
